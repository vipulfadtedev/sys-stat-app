...
const LineChart = () => {
  ...
  useEffect(() => {
    const run = () => {
      setData({});
      axios
        .get(`http://10.0.0.110:9090/stats/in-last-hour`)
        .then((res: { data: any }) => {
            ...
            (record: {
              ...
              if (counter) {
                if (counter[record.host]) {
                  counter[record.host].push({
                    x: epochToDate(record.timestamp),
                ...
                statusData[record.counter][record.host] = [
                  { x: epochToDate(record.timestamp), y: Number(record.value) },
                ];
              }
            }
          );
